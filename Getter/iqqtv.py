...
def main(number, appoint_url='', log_info='', req_web='', language='zh_cn'):
    ...
    iqqtv_website = config.get('iqqtv_website')
    if iqqtv_website:
    ...
    try:                                                                        # 捕获主动抛出的异常
        if not real_url:
            ...
            if not re.search(r'\d+[-_]\d+', number):
                number1 = ' ' + number.replace('FC2', '').replace('-PPV', '')
            real_url = html.xpath("//h3[@class='one_name ga_name' and (contains(text(), $number)) and not (contains(text(), '克破')) and not (contains(text(), '无码流出')) and not (contains(text(), '無碼流出')) and not (contains(text(), '無修正'))]/../@href", number=number1)
