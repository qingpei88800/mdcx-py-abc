...
def get_web_number(html):
    result = html.xpath('//p/span[contains(text(), "番號") or contains(text(), "番号")]/../span[@id="ContentPlaceHolder1_Label_barcode"]/text()')
...
def main(number, appoint_url='', log_info='', req_web='', language='zh_cn'):
    ...
    try:                                                                        # 捕获主动抛出的异常
        if not real_url:
            ...
            real_url = html.xpath("//h3[@class='one_name ga_name' and contains(text(), $number1) and not(contains(text(), '克破'))]/../@href", number1=number2)
