...
from Function.getInfo import long_name, remove_escape_string
from Getter import theporndb_movies
...
def main(number, appoint_url='', log_info='', req_web='', language='zh_cn', file_path='', appoint_number=''):
    ...
    api_token = config.get('theporndb_api_token')
    theporndb_no_hash = config.get('theporndb_no_hash')
    ...
    try:                                                                        # 捕获主动抛出的异常
        ...
        if not real_url:
            ...
            try:
                if not theporndb_no_hash:
                    ...
                    if hash_data:
                        number, title, outline, actor, all_actor, cover_url, poster_url, trailer, release, year, runtime, tag, director, series, studio, publisher, real_url = read_data(hash_data)
            except:
                pass
        ...
        if real_url and not hash_data:
            ...
            if real_data:
                number, title, outline, actor, all_actor, cover_url, poster_url, trailer, release, year, runtime, tag, director, series, studio, publisher, real_url = read_data(real_data)
            else:
        ...
        # print(traceback.format_exc())
        req_web = req_web + '(%ss) ' % (round((time.time() - start_time), ))
        return theporndb_movies.main(number, appoint_url=appoint_url, log_info=log_info, req_web=req_web, language='zh_cn', file_path=file_path, appoint_number=appoint_number)
...
if __name__ == '__main__':
    ...
    print(main('Evolvedfights.20.10.30', file_path='AARM-018 - 2021-09-28 - 未知演员 - アロマ企画，アロマ企画/evolvedfights.20.10.30.kay.carter.vs.nathan.bronson.mp4'))
