...
import json
from Function.getHtml import get_html, post_html, check_url, get_dmm_trailer
import urllib3
...
def main(number, appoint_url='', log_info='', req_web='', language='jp', file_path=''):
    ...
    try:
        ...
        if not real_url or 'tv.dmm.com' in real_url:
            ...
            log_info += web_info + debug_info
            result, title, outline, actor, poster_url, cover_url, tag, runtime, score, series, director, studio, publisher, extrafanart, trailer, year = get_tv_com_data(number_00)
            if not result:
                debug_info = '数据获取失败: %s ' % title
                log_info += web_info + debug_info
                raise Exception(debug_info)
        elif 'tv.dmm.co.jp' in real_url:
            result, title, outline, actor, poster_url, cover_url, tag, runtime, score, series, director, studio, publisher, extrafanart, trailer, year = get_tv_jp_data(real_url)
            if not result:
                debug_info = '数据获取失败: %s ' % title
                log_info += web_info + debug_info
                raise Exception(debug_info)
