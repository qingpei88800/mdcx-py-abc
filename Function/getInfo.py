...
def is_uncensored(number):
    ...
    key_start_word = ['BT-', 'CT-', 'EMP-', 'CCDV-', 'CWP-', 'CWPBD-', 'DSAM-', 'DRC-', 'DRG-', 'GACHI-', 'heydouga', 'JAV-', 'LAF-', 'LAFBD-', 'HEYZO-', 'KTG-', 'KP-', 'KG-', 'LLDV-', 'MCDV-', 'MKD-', 'MKBD-', 'MMDV-', 'NIP-', 'PB-', 'PT-', 'QE-', 'RED-', 'RHJ-', 'S2M-', 'SKY-', 'SKYHD-', 'SMD-', 'SSDV-', 'SSKP-', 'TRG-', 'TS-', 'xxx-av-', 'YKB-', 'bird', 'bouga']
...
def is_suren(number):
    if re.search(r'\d{3,}[A-Z]+-\d{2}', number.upper()) or 'SIRO' in number.upper():
        return True
    for key in cf.get_config().get('suren_dic').keys():
        if number.upper().startswith(key):
            return True
    return False


# ========================================================================根据番号获取车牌中的字母前缀，比如 ABP-100 的前缀是 ABP
...
# ========================================================================根据番号获取车牌中的字母前缀，比如 ABP-100 的首字母是 A
...
def long_name(short_name):
    long_name = cf.get_config().get('oumei_name').get(short_name.lower())
...
def remove_escape_string(filename, replace_char=''):
    ...
    for string in cf.get_config().get('escape_string_list'):
        if string:
...
def get_file_number(filepath):
    ...
    if 'MYWIFE' in filename and re.search(r'NO\.\d*', filename):                                # 提取 mywife No.1111
        ...
        return file_number
    elif re.findall(r'([A-Z0-9_]{2,})[-\.]{1}2?0?(\d{2}[-\.]\d{2}[-\.]\d{2})', oumei_filename): # 提取欧美番号 sexart.11.11.11
        ...
        for key, value in cf.get_config().get('suren_dic').items():
